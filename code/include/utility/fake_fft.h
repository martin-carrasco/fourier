...
/* dj_dfft.h - public domain FFT library
by Jonathan Dupuy
...
template <typename T>
using dfft_arg = std::vector<std::complex<T>>;

// FFT direction specifier
enum class dfft_dir { DIR_FWD = +1, DIR_BWD = -1 };

// FFT routines
template <typename T>
dfft_arg<T> dfft1d(const dfft_arg<T>& xi, const dfft_dir& dir);
template <typename T>
dfft_arg<T> dfft2d(const dfft_arg<T>& xi, const dfft_dir& dir);
template <typename T>
dfft_arg<T> dfft3d(const dfft_arg<T>& xi, const dfft_dir& dir);

// GPU FFT routines (float precision only)
dfft_arg<float> dfft1d_gpu(const dfft_arg<float>& xi, const dfft_dir& dir);
dfft_arg<float> dfft2d_gpu(const dfft_arg<float>& xi, const dfft_dir& dir);
dfft_arg<float> dfft3d_gpu(const dfft_arg<float>& xi, const dfft_dir& dir);

// GPU FFT routines (for advanced users: create an OpenGL context yourself)
dfft_arg<float> dfft1d_gpu_glready(const dfft_arg<float>& xi, const dfft_dir& dir);
dfft_arg<float> dfft2d_gpu_glready(const dfft_arg<float>& xi, const dfft_dir& dir);
dfft_arg<float> dfft3d_gpu_glready(const dfft_arg<float>& xi, const dfft_dir& dir);
...
dfft_arg<T> dfft1d(const dfft_arg<T>& xi, const dfft_dir& dir) {
    ...
    for (int j = 0; j < cnt; ++j) xo[j] = nrm * xi[bitr(j, msb)];
    ...
    for (int i = 0; i < msb; ++i) {
        ...
        for (int j2 = 0; j2 < (cnt / 2); ++j2)
            for (int j1 = 0; j1 < (cnt / 2); ++j1) {
                ...
                // FFT-X
                std::complex<T> z11 =
                    std::polar(T(1), ang * T(i11 ^ bw));  // left rotation
                std::complex<T> z12 =
                    std::polar(T(1), ang * T(i12 ^ bw));  // right rotation
                ...
                // FFT-Y
                std::complex<T> z21 =
                    std::polar(T(1), ang * T(i21 ^ bw));  // top rotation
                std::complex<T> z22 =
                    std::polar(T(1), ang * T(i22 ^ bw));  // bottom rotation
        ...
        for (int j3 = 0; j3 < (cnt / 2); ++j3)
            for (int j2 = 0; j2 < (cnt / 2); ++j2)
                for (int j1 = 0; j1 < (cnt / 2); ++j1) {
                    ...
                    // FFT-Z
                    std::complex<T> z31 =
                        std::polar(T(1), ang * T(i31 ^ bw));  // front rotation
                    std::complex<T> z32 =
                        std::polar(T(1), ang * T(i32 ^ bw));  // back rotation
...
#define DJGL_LIST                                                            \
    ...
    GLE(void, TexStorage1D, GLenum target, GLsizei levels,                   \
        GLenum internalformat, GLsizei width)                                \
    GLE(void, TexStorage2D, GLenum target, GLsizei levels,                   \
        GLenum internalformat, GLsizei width, GLsizei height)                \
    GLE(void, TexStorage3D, GLenum target, GLsizei levels,                   \
        GLenum internalformat, GLsizei width, GLsizei height, GLsizei depth) \
    GLE(void, BindImageTexture, GLuint unit, GLuint texture, GLint level,    \
        GLboolean layered, GLint layer, GLenum access, GLenum format)        \
    GLE(void, DispatchCompute, GLuint num_groups_x, GLuint num_groups_y,     \
        GLuint num_groups_z)                                                 \
    ...
    GLE(void, TexSubImage1D, GLenum target, GLint level, GLint xoffset,      \
        GLsizei width, GLenum format, GLsizei imageSize, const void* data)   \
    GLE(void, TexSubImage2D, GLenum target, GLint level, GLint xoffset,      \
        GLint yoffset, GLsizei width, GLsizei height, GLenum format,         \
        GLenum type, const void* pixels)                                     \
    GLE(void, TexSubImage3D, GLenum target, GLint level, GLint xoffset,      \
        GLint yoffset, GLint zoffset, GLsizei width, GLsizei height,         \
        GLsizei depth, GLenum format, GLenum type, const void* pixels)       \
    GLE(void, GetTexImage, GLenum target, GLint level, GLenum format,        \
...
#define GLE(ret, name, ...)                         \
    typedef ret GLAPIENTRY name##Proc(__VA_ARGS__); \
...
class OpenGLContext {
   public:
    OpenGLContext() {
        ...
        // Create window
        HWND hwnd =
            CreateWindowExA(WS_EX_CLIENTEDGE, windowClassName, windowClassName,
        ...
        int dummy;
        GLXFBConfig cfg = glXGetFBConfigs(dpy, 0, &dummy)[0];
...
dfft_arg<float> dfft1d_gpu(const dfft_arg<float>& xi, const dfft_dir& dir) {
    ...
    return dfft1d_gpu_glready(xi, dir);
}
...
dfft_arg<float> dfft1d_gpu_glready(const dfft_arg<float>& xi, const dfft_dir& dir) {
    ...
    // setup GPU Kernel
    {
        ...
        const GLchar* shaderSrc[] = {"#version 450 core\n", "#define FFT_1D\n",
                                     s_ComputeShaderSrc};
        ...
        glShaderSource(shader, sizeof(shaderSrc) / sizeof(shaderSrc[0]),
                       shaderSrc, NULL);
...
dfft_arg<float> dfft2d_gpu(const dfft_arg<float>& xi, const dfft_dir& dir) {
    ...
    return dfft2d_gpu_glready(xi, dir);
}
...
dfft_arg<float> dfft3d_gpu(const dfft_arg<float>& xi, const dfft_dir& dir) {
    ...
    return dfft3d_gpu_glready(xi, dir);
}
...
dfft_arg<float> dfft3d_gpu_glready(const dfft_arg<float>& xi, const dfft_dir& dir) {
    ...
    glTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, 0, cnt, cnt, cnt, GL_RG, GL_FLOAT,
                    &xo[0]);
    glBindImageTexture(0, gl.texture, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RG32F);
